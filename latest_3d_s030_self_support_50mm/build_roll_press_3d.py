...
# ---------------------------------------------------------------------------
WORKDIR = r"E:\abaqus\3Dnihe2.0"
MODEL_NAME = "RP3D_Self_DPC_S030_50mm_Roll10mm_dt5e6_CoarseRoller_V2p2"
JOB_NAME = "RP3D_Self_DPC_S030_50mm_Roll10mm_dt5e6_CoarseRoller_V2p2"
...
# Drucker-Prager/Cap parameters for the active coating.  The cap hardening
# pressure is the original industrial-scale curve multiplied by S = 0.3.
# Units: MPa for cohesion/cap pressure, degree for friction angle.
DCP_COHESION = 4.0
DCP_FRICTION_ANGLE = 65.0
DCP_ECCENTRICITY = 0.8
DCP_INITIAL_CAP_POSITION = 18.0
...
ROLLING_DISTANCE = 10.0
CLAMP_DOWN_TIME = 0.05
HOLD_TIME = 0.05
ROLLING_RAMP_TIME = 0.05
ROLLING_TIME = ROLLING_DISTANCE / UPPER_LINE_SPEED
MASS_SCALING_MODE = "MODEL"  # NONE, LOCAL_ACTIVE, or MODEL
...
SHEET_X_PARTITIONS = (-35.0, -15.0, -5.0, -1.0)
SHEET_X_COARSE_SIZE = 1.5
SHEET_X_TRANSITION_SIZE = 1.25
SHEET_X_FINE_SIZE = 0.75
SHEET_X_NIP_SIZE = 0.75
SHEET_WIDTH_SIZE = 5.0
ACTIVE_THICKNESS_ELEMS = 5
ROLLER_GLOBAL_SIZE = 25.0
ROLLER_FACE_SIZE = 10.0
...
def make_materials(model):
    roller = model.Material(name="Roller_E193GPa")
    roller.Density(table=((7.93e-9,),))
    roller.Elastic(table=((193000.0, 0.20),))
...
def make_steps_bcs_loads_outputs(model, root):
    model_mass_scaling = ((SEMI_AUTOMATIC, MODEL, AT_BEGINNING, 0.0,
                           MASS_SCALING_TARGET_DT, BELOW_MIN, 0, 0,
                           0.0, 0.0, 0, None),)
    model.ExplicitDynamicsStep(
        ...
        massScaling=model_mass_scaling)
    model.ExplicitDynamicsStep(
        name="Hold_Clamp",
        previous="Clamp_Down",
...
def main():
    ...
    elif MASS_SCALING_MODE == "MODEL":
        print("Whole-model mass scaling target dt: %.3e s" %
              MASS_SCALING_TARGET_DT)
    else:
