# -*- coding: utf-8 -*-
# 2D 平面应变静力平板压缩-卸载验证。
# 用 Abaqus/Standard 避免显式卸载时的动态释放冲击，只检查材料参数
# 在压到 135 um 后是否能保留永久压实并产生小范围回弹。
...
MODEL_NAME = "FlatUnload_2D_DPC_S030_E6500Nu001_StaticContactOff"
JOB_NAME = MODEL_NAME
...
def make_sheet_part(model):
    ...
    elem = mesh.ElemType(elemCode=CPE4, elemLibrary=STANDARD)
...
def make_interactions(model, root, top, bottom):
    ...
    prop.TangentialBehavior(formulation=PENALTY,
                            ...
                            fraction=0.005)
    model.SurfaceToSurfaceContactStd(name="TopPlate_to_Film",
                                     createStepName="Initial",
                                     main=top.surfaces["PlateSurface"],
                                     secondary=root.surfaces["Sheet_Top"],
                                     sliding=FINITE,
                                     thickness=ON,
                                     interactionProperty=prop.name,
                                     adjustMethod=NONE,
                                     initialClearance=OMIT)
    model.SurfaceToSurfaceContactStd(name="BottomPlate_to_Film",
                                     createStepName="Initial",
                                     main=bottom.surfaces["PlateSurface"],
                                     secondary=root.surfaces["Sheet_Bottom"],
                                     sliding=FINITE,
                                     thickness=ON,
                                     interactionProperty=prop.name,
                                     adjustMethod=NONE,
...
def make_steps_bcs_outputs(model, root):
    model.StaticStep(name="Compress_Down",
                     previous="Initial",
                     nlgeom=ON,
                     maxNumInc=200,
                     initialInc=0.02,
                     minInc=1.0e-8,
                     maxInc=0.05)
    model.StaticStep(name="Hold_135um",
                     ...
                     minInc=1.0e-8,
                     maxInc=0.2)
    model.StaticStep(name="Unload_Free",
                     previous="Hold_135um",
                     nlgeom=ON,
                     maxNumInc=200,
                     initialInc=0.02,
    ...
    model.DisplacementBC(name="BC_TopPlate_U2",
                         createStepName="Compress_Down",
                         region=top_rp,
                         u1=UNSET, u2=-COMPRESS_DISP, ur3=UNSET)
    model.boundaryConditions["BC_TopPlate_U2"].setValuesInStep(
        stepName="Hold_135um", u2=-COMPRESS_DISP)
    model.boundaryConditions["BC_TopPlate_U2"].setValuesInStep(
        stepName="Unload_Free", u2=UNLOAD_LIFT)
    model.DisplacementBC(name="BC_Sheet_XY_Anchor",
                         createStepName="Initial",
                         region=root.sets["Bottom_Left_Node"],
                         u1=0.0, u2=0.0, ur3=UNSET)

    # 卸载阶段撤掉压板接触，只保留一个节点锚定防止刚体漂移。
    # 这样得到的是无接触压力条件下的自由残余厚度。
    model.interactions["TopPlate_to_Film"].deactivate("Unload_Free")
    model.interactions["BottomPlate_to_Film"].deactivate("Unload_Free")
...
def make_job(model):
    ...
    return mdb.Job(name=JOB_NAME,
                   model=model.name,
                   description="Static 2D flat compression-unload check for S=0.3 E6500 nu0.01 DPC film",
                   type=ANALYSIS,
...
def main():
    ...
    print("Created static model: %s" % MODEL_NAME)
